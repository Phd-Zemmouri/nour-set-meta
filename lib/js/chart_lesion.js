...
const lesionPieData = [
	{ name: 'Q1[01-25%]', y: 675, color: '#90E0EF' },
	{ name: 'Q2[26-50%]', y: 339, color: '#00B4D8' },
	{ name: 'Q3[51-75%]', y: 112, color: '#0077B6' },
	{ name: 'Q4[76-100%]', y: 23, color: '#03045E' },
]
	...
	Highcharts.chart({
		...
		legend: {
			...
			labelFormatter: function () {
				return `${this.name} <span style="color${this.color};font-weight:bold">: ${this.y}</span>`
			},
		},
		...
		plotOptions: {
			column: {
				borderWidth: 0,
				borderRadius: 4,
				dataLabels: { enabled: true, format: '{point.y:,.0f}', style: { fontWeight: 'bold', fontSize: '20px' } },
			},
...
function get_Chart_Lesion_D() {
	Highcharts.chart({
		...
		tooltip: {
			shared: false,
			useHTML: true,
			headerFormat: '<span style="font-size:20px;font-weight:bold">{point.key}</span><br/>',
