...
function get_Chart_Variant() {
	get_Chart_Variant_A(data_Variant)
	get_Chart_Variant_B(data_Variant)
	get_Chart_Variant_C(data_Variant)
}
...
function get_Chart_Variant_A(data) {
	...
	const chart = new Highcharts.Chart(MyOptions)
}

function get_Chart_Variant_B(data) {
	const chart_data = variants.map((variant) => {
		const { month, ...new_element } = variant // exlures month
		new_element['y'] = data.filter((ele) => ele.variant.endsWith(variant.name)).reduce((sum, ele) => sum + ele.cases, 0)
...
function get_Chart_Variant_C0(data) {
	let categories = data.map((ele) => ele.month).sort()
	let series = variants.map((variant) => {
		// Methode 1
		const { month, ...new_element } = variant // exlures month
	...
	const variantsPieData = variants.map((variant) => {
		const { month, ...new_element } = variant // exlures month
		new_element['y'] = data.filter((ele) => ele.variant.endsWith(variant.name)).reduce((sum, ele) => sum + ele.cases, 0)
	...
	const MyOptions = {
		...
		exporting: {
			enabled: true,
			filename: 'Variant_distribution_C',
			sourceWidth: 1000,
			sourceHeight: 500,
		},
		plotOptions: {
		...
		series: [
			...
			{
				name: 'variants distribution',
				type: 'pie',
				legendType: 'point',

				data: variantsPieData.map((item) => ({
					...item,
...
function get_Chart_Variant_C(data) {
	/* ==========================
	   Prepare Data
	========================== */

	const categories = [...new Set(data.map((ele) => ele.month))].sort()

	const areaSeries = variants.map((variant) => {
		const { month, ...newVariant } = variant
	...
	const pieData = variants.map((variant) => {
		...
		return {
			...newVariant,
			y: data.filter((ele) => ele.variant.endsWith(variant.name)).reduce((sum, ele) => sum + ele.cases, 0),
		}
	})
	...
	Highcharts.chart({
		...
		xAxis: {
			...
			labels: {
				...
				formatter: function () {
					const currentMonth = this.value
					const variant = variants.find((v) => v.month === currentMonth)
			...
			series: {
				...
				dataLabels: {
					...
					formatter: function () {
						...
						return pinnacles.includes(this.key) ? this.y.toLocaleString('en-US').replace(',', ' ') : ''
					},
		...
		series: [
...

			{
				...
				data: pieData.map((item) => ({
					...item,
					name: `${item.name} : ${item.y.toLocaleString()}`,
				})),

				tooltip: {
					pointFormat: '<b>{point.name}</b>: {point.y:,.0f} cases ({point.percentage:.1f}%)<br/>',
				},
			},
		],
	})
